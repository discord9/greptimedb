...
///
/// A timestamp timezone difference is the only intentional exception: its unit
/// must match, and replacing the actual timestamp datatype with the expected
/// datatype must make the complete field equal.
...
mod tests {
    ...
    async fn qbs_merge_scan_remote_schema_identity_canonical_two_regions() {
        ...
        let batches = collect_merge_scan(merge_scan_exec(
            vec![
                (RegionId::new(1024, 1), canonical_batch()),
                (RegionId::new(1024, 2), canonical_batch()),
            ],
            expected_int64_schema(),
            1,
        ))
    ...
    async fn qbs_merge_scan_remote_schema_identity_allows_timestamp_timezone_only_patch() {
        ...
        let timestamp = batches[0]
            ...
            .expect("column ts must be a millisecond timestamp");
        assert_eq!(timestamp.value(0), 1002);
    }
    ...
    fn merge_scan_remote_schema_identity_rejects_field_metadata_and_nullability_mismatch() {
        ...
        let field_metadata_mismatch = ArrowSchema::new_with_metadata(
            vec![
                expected
                    .field(0)
                    .as_ref()
        ...
        assert!(validate_remote_schema(&expected, &field_metadata_mismatch, "test").is_err());
    ...
    fn merge_scan_remote_schema_identity_returns_arrow_schema_error() {
        let expected = expected_int64_schema();
        let actual = ArrowSchema::new_with_metadata(
            vec![Field::new("other", TestArrowDataType::Int64, false)],
            expected.metadata().clone(),
        );
