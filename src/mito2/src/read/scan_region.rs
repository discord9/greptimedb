...
pub struct ScanInput {
    ...
    /// (`experimental_preserve_row_sequence` on an append-only table) and the scan
...
impl ScanInput {
    ...
    /// Sets the exact sequence range applied row-level on SST reads.
    #[must_use]
...
/// Returns true if every SST file in the `version` was written with preserved
/// per-row sequences (the `experimental_preserve_row_sequence` region option).
///
/// This is the file-level precondition for exact row-level sequence filtering on
/// SST data. If any file lacks the marker (e.g. legacy files written before the
/// option was enabled), the region must keep the sequence fences so exact scans
/// fall back instead of silently crossing the stale/upper fence.
pub(crate) fn all_files_preserve_row_sequence(version: &crate::region::version::Version) -> bool {
    version.ssts.levels().iter().all(|level| {
        level
            .files
            .values()
...
/// - the region preserves per-row sequences (`experimental_preserve_row_sequence`),
///   and
/// - every SST file in the version carries the preserved-sequence marker.
///
...
pub(crate) fn exact_sequence_range(
    ...
    if !request.exact_sequence_range
        ...
        || !version.options.experimental_preserve_row_sequence
        || !all_files_preserve_row_sequence(version)
    {
        return None;
    }
