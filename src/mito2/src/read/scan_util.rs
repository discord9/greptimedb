    ...
    try_stream! {
        ...
        for range in ranges {
            ...
            while let Some(record_batch) = reader.next_batch().await? {
                ...
                // Exact row-level sequence filtering for SST data. Applied only
                // when the region preserves per-row sequences
                // (experimental_preserve_row_sequence) and the file carries the
                // preserved-sequence marker; see ScanInput::sequence_range.
...
mod sequence_filter_tests {
    ...
    fn test_filter_flat_batch_by_sequence_exact_range() {
        ...
        // (0, 2] keeps sequences 1 and 2 (tags 0 and 1): lower exclusive, upper inclusive.
        let out = filter_flat_batch_by_sequence(
            b.clone(),
            Some(SequenceRange::GtLtEq { min: 0, max: 2 }),
            true,
        )
        .unwrap();
        assert_eq!(remaining_tags(&out.unwrap()), vec!["0", "1"]);
        ...
        // LtEq only.
        let out =
            filter_flat_batch_by_sequence(b.clone(), Some(SequenceRange::LtEq { max: 2 }), true)
                .unwrap();
        assert_eq!(remaining_tags(&out.unwrap()), vec!["0", "1"]);

        // Gt only.
        let out =
            filter_flat_batch_by_sequence(b.clone(), Some(SequenceRange::Gt { min: 2 }), true)
                .unwrap();
        assert_eq!(remaining_tags(&out.unwrap()), vec!["2", "3"]);

        // Empty batch with a range.
        let empty = batch(&[]);
        let out = filter_flat_batch_by_sequence(
            empty,
