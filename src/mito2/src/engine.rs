        ...
        if request.exact_sequence_range {
            ...
            if exact_sequence_range.is_none() {
                ...
                return SequenceRangeUnsupportedSnafu {
                    ...
                    reason: if version.options.experimental_preserve_row_sequence {
                        "region has files without preserved per-row sequences".to_string()
                    } else {
                        "region does not preserve per-row sequences (experimental_preserve_row_sequence is off)".to_string()
                    },
                }
                .fail();
            }
