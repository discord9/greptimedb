...
- **小 build-side 会被 InListExpr 短路**：DataFusion 的 `HashJoinExec` 在 build-side distinct key 数低于 `hash_join_inlist_pushdown_max_distinct_values`（默认 256）时，不会生成 `HashTableLookupExpr`，而是直接用 `InListExpr (IN (SET))`，导致 Bloom 编码全部跳过。因此 Bloom E2E 测试的 build-side 必须 >302 个 key。
