...
impl Phase3E2eScenarioKind {
    fn as_seed_value(&self) -> &'static str {
        match self {
            ...
            Self::GcIdempotency => "gc_idempotency",
        }
    }
    ...
    fn from_seed_value(value: &str) -> Result<Self, String> {
        match value {
            ...
            "gc_idempotency" => Ok(Self::GcIdempotency),
...
#[derive(Debug)]
pub struct Phase3GcIdempotencyEvidence {
    pub gc_report_first: GcReport,
    pub gc_report_second: GcReport,
    pub sst_after_first_gc: BTreeSet<String>,
    pub manifest_after_first_gc: BTreeSet<String>,
    pub sst_after_second_gc: BTreeSet<String>,
    pub manifest_after_second_gc: BTreeSet<String>,
    pub count_output_before_gc: String,
    pub count_output_after_first_gc: String,
    pub count_output_after_second_gc: String,
}
...
pub struct Phase3E2eEvidence {
    ...
    pub gc_idempotency_evidence: Option<Phase3GcIdempotencyEvidence>,
...
pub async fn run_phase3_e2e_gc_cycle(input: Phase3E2eInput) -> Phase3E2eEvidence {
    ...
    if input.scenario_kind == Phase3E2eScenarioKind::GcIdempotency {
        return run_gc_idempotency_scenario(input).await;
    }
    ...
    generate_ssts(&instance, &table_name, flush_rounds, input.table_shape).await;
    ...
    assert_eq!(sst_before_compaction.len(), flush_rounds * regions.len());
...
pub fn validate_phase3_e2e_evidence(evidence: &Phase3E2eEvidence) -> Result<(), String> {
    ...
    validate_gc_idempotency_evidence(evidence)?;
...
fn phase3_scenario_trace_step(scenario_kind: Phase3E2eScenarioKind) -> &'static str {
    match scenario_kind {
        ...
        Phase3E2eScenarioKind::GcIdempotency => {
            "run gc_idempotency scenario: first GC deletes, second GC no-op"
        }
    }
}
...
//   9. Run real batch GC
//  10. Validate strict invariants (no storage-diff fallback, deleted_indexes non-empty)
...
// ---------------------------------------------------------------------------
//  run_gc_idempotency_scenario
// ---------------------------------------------------------------------------
//
// Flow (oracle-reviewed):
//   1. Create append-mode table (supports single/multi region)
//   2. Generate SSTs, record sst_before_compaction and regions
//   3. Compact table, record sst_after_compaction and manifest_after_compaction
//   4. Prove compaction creates obsolete parquet pressure
//   5. First BatchGcProcedure → deletes obsolete parquet
//   6. Second BatchGcProcedure → must be no-op
//   7. Build evidence: gc_report = first report, sst_after_gc = second snapshot
//   8. Generic validator still works (first report deletions match object-store diff)
async fn run_gc_idempotency_scenario(input: Phase3E2eInput) -> Phase3E2eEvidence {
    common_telemetry::init_default_ut_logging();
    ...
    let table_name = format!("phase3_gc_table_{}", input.seed);
    let flush_rounds = input.flush_rounds.max(4);
    let full_file_listing = true;

    // --- Step 1: Create append-mode table ---
    create_append_mode_table(&instance, &table_name, input.table_shape).await;

    // --- Step 2: Generate SSTs ---
    generate_ssts(&instance, &table_name, flush_rounds, input.table_shape).await;
    ...
    // --- Step 3: Compact table ---
    compact_table(&instance, &table_name, input.compaction_wait_secs).await;

    let sst_after_compaction = cluster.list_sst_files_from_all_datanodes().await;
    let manifest_after_compaction = cluster.list_sst_files_from_manifests().await;
    assert_eq!(
        sst_after_compaction.len(),
        (flush_rounds + 1) * regions.len()
    );

    // --- Step 4: Prove compaction creates obsolete parquet pressure ---
    let obsolete_after_compaction: BTreeSet<_> = sst_after_compaction
        .difference(&manifest_after_compaction)
        .cloned()
        .collect();
    assert!(
        !obsolete_after_compaction.is_empty(),
        "compaction must create obsolete parquet files: sst={} manifest={}",
        sst_after_compaction.len(),
        manifest_after_compaction.len(),
    );

    // --- Step 5: Capture count before GC ---
    let count_output_before_gc = query_count_output(&instance, &table_name).await;

    // --- Step 6: First BatchGcProcedure ---
    let gc_report_first = run_batch_gc(&cluster, regions.clone(), full_file_listing).await;

    let sst_after_first_gc = cluster.list_sst_files_from_all_datanodes().await;
    let manifest_after_first_gc = cluster.list_sst_files_from_manifests().await;
    let count_output_after_first_gc = query_count_output(&instance, &table_name).await;

    // --- Step 7: Second BatchGcProcedure (must be no-op) ---
    let gc_report_second = run_batch_gc(&cluster, regions.clone(), full_file_listing).await;

    let sst_after_second_gc = cluster.list_sst_files_from_all_datanodes().await;
    let manifest_after_second_gc = cluster.list_sst_files_from_manifests().await;
    let count_output_after_second_gc = query_count_output(&instance, &table_name).await;

    info!(
        "gc_idempotency scenario finished: seed={}, table={}, first_gc_deleted={:?}, second_gc_deleted={:?}",
        input.seed,
        table_name,
        collect_deleted_file_ids(&gc_report_first),
        collect_deleted_file_ids(&gc_report_second),
    );

    let gc_idempotency_evidence = Phase3GcIdempotencyEvidence {
        gc_report_first: gc_report_first.clone(),
        gc_report_second: gc_report_second.clone(),
        sst_after_first_gc: sst_after_first_gc.clone(),
        manifest_after_first_gc: manifest_after_first_gc.clone(),
        sst_after_second_gc: sst_after_second_gc.clone(),
        manifest_after_second_gc: manifest_after_second_gc.clone(),
        count_output_before_gc,
        count_output_after_first_gc,
        count_output_after_second_gc: count_output_after_second_gc.clone(),
    };
    ...
    let evidence = Phase3E2eEvidence {
        ...
        sst_before_compaction,
        sst_after_compaction,
        sst_after_gc: sst_after_second_gc.clone(),
        manifest_after_gc: manifest_after_second_gc.clone(),
        gc_report: gc_report_first.clone(),
        repartition_evidence: None,
        pre_gc_protection_evidence: None,
        admin_gc_dropped_evidence: None,
        post_migration_admin_gc_evidence: None,
        build_index_gc_evidence: None,
        gc_idempotency_evidence: Some(gc_idempotency_evidence),
        follower_required_files: HashSet::new(),
        count_output: count_output_after_second_gc.clone(),
        replay_trace: vec![
            format!(
                "create {} append-mode table",
                input.table_shape.as_seed_value()
            ...
            "compact table to create removable SSTs".to_string(),
            format!(
                "first BatchGcProcedure(full_file_listing={})",
                full_file_listing
            ),
            format!(
                "second BatchGcProcedure(full_file_listing={})",
                full_file_listing
            ),
            phase3_scenario_trace_step(input.scenario_kind).to_string(),
        ],
    };

    validate_phase3_e2e_evidence(&evidence)
        .unwrap_or_else(|err| panic!("Phase 3 GcIdempotency invariant violation: {err}"));
...
fn validate_gc_idempotency_evidence(evidence: &Phase3E2eEvidence) -> Result<(), String> {
    if evidence.scenario_kind != Phase3E2eScenarioKind::GcIdempotency {
        if evidence.gc_idempotency_evidence.is_some() {
            return Err(format!(
                "non-gc-idempotency scenario {} unexpectedly recorded gc-idempotency evidence; {} trace={:?}",
                evidence.scenario_kind.as_seed_value(),
                evidence.concise_summary(),
                evidence.replay_trace,
            ));
    ...
    let Some(ev) = &evidence.gc_idempotency_evidence else {
        return Err(format!(
            "gc_idempotency scenario missing gc-idempotency evidence; {} trace={:?}",
            evidence.concise_summary(),
            evidence.replay_trace,
        ));
    };

    // (1) requested regions == first processed regions == second processed regions
    let requested: HashSet<RegionId> = evidence.regions.iter().copied().collect();
    if requested != ev.gc_report_first.processed_regions {
        return Err(format!(
            "first GC processed_regions {:?} != requested regions {:?}; evidence={:?}",
            ev.gc_report_first.processed_regions, requested, ev,
        ));
    }
    if requested != ev.gc_report_second.processed_regions {
        return Err(format!(
            "second GC processed_regions {:?} != requested regions {:?}; evidence={:?}",
            ev.gc_report_second.processed_regions, requested, ev,
        ));
    }

    // (2) first and second need_retry_regions must both be empty
    if !ev.gc_report_first.need_retry_regions.is_empty() {
        return Err(format!(
            "first GC has unexpected need_retry_regions: {:?}; evidence={:?}",
            ev.gc_report_first.need_retry_regions, ev,
        ));
    }
    if !ev.gc_report_second.need_retry_regions.is_empty() {
        return Err(format!(
            "second GC has unexpected need_retry_regions: {:?}; evidence={:?}",
            ev.gc_report_second.need_retry_regions, ev,
        ));
    }

    // (3) flatten(first.deleted_files) non-empty
    let first_deleted = collect_deleted_file_ids(&ev.gc_report_first);
    if first_deleted.is_empty() {
        return Err(format!(
            "first GC must delete at least one parquet file; evidence={:?}",
            ev,
        ));
    }

    // (4) flatten(first.deleted_indexes) empty
    let first_deleted_indexes: HashSet<_> = ev
        .gc_report_first
        .deleted_indexes
        .values()
        .flat_map(|v| v.iter())
        .collect();
    if !first_deleted_indexes.is_empty() {
        return Err(format!(
            "first GC unexpectedly deleted indexes: {:?}; evidence={:?}",
            first_deleted_indexes, ev,
        ));
    }

    // (5) flatten(second.deleted_indexes) empty
    let second_deleted_indexes: HashSet<_> = ev
        .gc_report_second
        .deleted_indexes
        .values()
        .flat_map(|v| v.iter())
        .collect();
    if !second_deleted_indexes.is_empty() {
        return Err(format!(
            "second GC unexpectedly deleted indexes: {:?}; evidence={:?}",
            second_deleted_indexes, ev,
        ));
    }

    // (6) flatten(second.deleted_files) empty (note: map may contain empty vecs)
    let second_deleted = collect_deleted_file_ids(&ev.gc_report_second);
    if !second_deleted.is_empty() {
        return Err(format!(
            "second GC must be no-op (deleted_files non-empty): {:?}; evidence={:?}",
            second_deleted, ev,
        ));
    }

    // (7) sst_after_first_gc == sst_after_second_gc
    if ev.sst_after_first_gc != ev.sst_after_second_gc {
        return Err(format!(
            "sst snapshots diverged between first and second GC; first={} second={}; evidence={:?}",
            ev.sst_after_first_gc.len(),
            ev.sst_after_second_gc.len(),
            ev,
        ));
    }

    // (8) manifest_after_first_gc == manifest_after_second_gc
    if ev.manifest_after_first_gc != ev.manifest_after_second_gc {
        return Err(format!(
            "manifest snapshots diverged between first and second GC; first={} second={}; evidence={:?}",
            ev.manifest_after_first_gc.len(),
            ev.manifest_after_second_gc.len(),
            ev,
        ));
    }

    // (9) count_outputs contain expected row count
    let expected_row_count =
        (evidence.flush_rounds * evidence.table_shape.rows_per_flush()).to_string();
    for (label, output) in [
        ("before_gc", &ev.count_output_before_gc),
        ("after_first_gc", &ev.count_output_after_first_gc),
        ("after_second_gc", &ev.count_output_after_second_gc),
    ] {
        if !output.contains(&expected_row_count) {
            return Err(format!(
                "gc_idempotency count_output_{label} does not contain expected '{expected_row_count}'; output=\n{output}"
            ));
        }
    }

    // (10) manifest_after_second_gc FileIds subset of sst_after_second_gc FileIds
    let manifest_second_ids = collect_file_ids_from_paths(ev.manifest_after_second_gc.clone())?;
    let sst_second_ids = collect_file_ids_from_paths(ev.sst_after_second_gc.clone())?;
    if !manifest_second_ids.is_subset(&sst_second_ids) {
        let missing: Vec<_> = manifest_second_ids.difference(&sst_second_ids).collect();
        return Err(format!(
            "manifest_after_second_gc files missing from sst_after_second_gc: {:?}; evidence={:?}",
            missing, ev,
        ));
    }
...
mod tests {
    ...
    async fn test_phase3_e2e_smoke_full_listing_gc_cycle() {
        let evidence = run_phase3_e2e_gc_cycle(Phase3E2eInput {
            flush_rounds: 2,
            ..Phase3E2eInput::smoke(7)
        })
        .await;

        assert_eq!(evidence.regions.len(), 1);
        assert_eq!(evidence.flush_rounds, 4);
    ...
    #[tokio::test]
    #[ignore = "known production gap: full-listing GC does not enumerate region_dir/index/ yet, \
                so obsolete v0 .puffin files remain in storage and deleted_indexes is empty"]
    ...
    #[tokio::test]
    async fn test_phase3_e2e_gc_idempotency() {
        let evidence = run_phase3_e2e_gc_cycle(Phase3E2eInput {
            seed: 101,
            // The runner must raise this to 4 so direct replay/corpus inputs
            // still create enough TWCS compaction pressure for first-GC deletion.
            flush_rounds: 3,
            full_file_listing: true,
            compaction_wait_secs: 2,
            table_shape: Phase3E2eTableShape::MultiRegion,
            scenario_kind: Phase3E2eScenarioKind::GcIdempotency,
        })
        .await;

        let ev = evidence
            .gc_idempotency_evidence
            .as_ref()
            .expect("gc_idempotency evidence must be present");
        assert_eq!(evidence.flush_rounds, 4);

        // Basic evidence assertions
        let first_deleted = collect_deleted_file_ids(&ev.gc_report_first);
        assert!(
            !first_deleted.is_empty(),
            "first GC must delete at least one parquet file"
        );

        let second_deleted = collect_deleted_file_ids(&ev.gc_report_second);
        assert!(second_deleted.is_empty(), "second GC must be no-op");

        assert_eq!(
            ev.sst_after_first_gc, ev.sst_after_second_gc,
            "sst snapshots must be equal after idempotent GC"
        );
        assert_eq!(
            ev.manifest_after_first_gc, ev.manifest_after_second_gc,
            "manifest snapshots must be equal after idempotent GC"
        );

        // Requested regions set == first processed regions == second processed regions
        let requested: HashSet<RegionId> = evidence.regions.iter().copied().collect();
        assert_eq!(
            requested, ev.gc_report_first.processed_regions,
            "first GC processed_regions must match requested regions"
        );
        assert_eq!(
            requested, ev.gc_report_second.processed_regions,
            "second GC processed_regions must match requested regions"
        );

        // Both need_retry_regions must be empty
        assert!(ev.gc_report_first.need_retry_regions.is_empty());
        assert!(ev.gc_report_second.need_retry_regions.is_empty());

        // Count includes expected rows: flush_rounds * rows_per_flush = 4 * 4 = 16
        let expected_count = "16";
        assert!(
            ev.count_output_before_gc.contains(expected_count),
            "count before GC should contain '{expected_count}', got:\n{}",
            ev.count_output_before_gc,
        );
        assert!(
            ev.count_output_after_first_gc.contains(expected_count),
            "count after first GC should contain '{expected_count}', got:\n{}",
            ev.count_output_after_first_gc,
        );
        assert!(
            ev.count_output_after_second_gc.contains(expected_count),
            "count after second GC should contain '{expected_count}', got:\n{}",
            ev.count_output_after_second_gc,
        );

        // Top-level evidence fields
        assert_eq!(
            evidence.manifest_after_gc, evidence.sst_after_gc,
            "manifest and storage sets must be identical after idempotent GC"
        );
        assert_eq!(evidence.manifest_after_gc, ev.manifest_after_second_gc,);
        assert_eq!(evidence.sst_after_gc, ev.sst_after_second_gc,);
    }
}
